...
import callApi, { requestUploadFile } from "../../utils/apiCaller";
...
export const UploadFile_Request = (file, func) => {
  let form_data = new FormData();
  for (let i = 0; i < file.length; i++) {
    form_data.append("file", file[file.length - 1]);
  }
  return (dispatch) => {
    return requestUploadFile(api.uploadFile, {
      method: "POST",
      body: form_data,
    }).then(function (res) {
      if (res) {
